...
st.set_page_config(page_title="股東紀念品數據分析", page_icon="📊", layout="wide")
...
st.markdown('<div class="big-title">📊 股東紀念品數據分析平台</div>', unsafe_allow_html=True)
st.markdown("---")
...
if not gift_rows.empty:
    ...
    # 2. 建立一個乾淨的 DataFrame (去掉 index 重設，直接交給 streamlit 處理)
    df_to_show = gift_rows[available_cols].copy()
    ...
    try:
        ...
        # 如果 hide_index 報錯，改用舊方法
        st.write("發生錯誤，嘗試相容模式顯示...")
